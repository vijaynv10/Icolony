...
{
    const account = await db.DeliveryPerson.findOne({ username });
    if(account == null || account == ""){
        throw("Username is not valid. Please Add/Edit Delivery person account details from merchant login");
    }
...
async function getAccount(id) {
    ...
    const account = await db.DeliveryPerson.findById(id);
...
//Adding Route
async function addRoute({ shops,ownerid,routeupdatedby, ipAddress, origin}) {
    
    const merchant_account = await db.Subscription.findOne({ subscriptionuser:ownerid });
    if(!merchant_account.delivery)
        throw("Delivery subscription not found")  
    if(!DateCheck(merchant_account.deliveryvalidity))  
        throw("Delivery subscription expired")  
    
    const route = new db.Route(); 
    route.shops = shops;
    route.ownerid = ownerid;
    route.routeupdatedby = routeupdatedby;
    route.routecreated = Date.now();
    await route.save();
}  

//Editing route
async function editRoute({ shops,ownerid,routeid,routeupdatedby, ipAddress, origin}) {
    ...
    const route = await db.Route.findOne({ _id:routeid });
    route.shops = shops;
    route.routeupdatedby = routeupdatedby;
    route.routeupdated = Date.now();
    await route.save();
}  

// get route
async function getRoute({ routeid, ipAddress, origin}) {
    if (!db.isValidId(routeid)) throw 'Route not found';
